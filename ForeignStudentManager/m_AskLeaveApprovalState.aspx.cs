...
{
    public partial class m_AskLeaveApprovalState : System.Web.UI.Page
    {
        ...
        protected void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            string strSQL = "select max(ApprovalStateID) from AskLeaveApprovalState;";
            ...
            lblApprovalStateID.Text = n.ToString();
        ...
        protected void btnDelMany_Click(object sender, EventArgs e)
        {
            ...
            com.Connection = conn;
            conn.Open();
            for (int i = 0; i < grdAskLeaveApprovalState.Rows.Count; i++)
            {
                cb = (CheckBox)grdAskLeaveApprovalState.Rows[i].FindControl("chk_Select");
                if (cb.Checked)
                {
                    ustID = int.Parse(grdAskLeaveApprovalState.Rows[i].Cells[1].Text);
                    if (n == 0)
                    {
                        strSQL = string.Format("delete from AskLeaveApprovalState where ApprovalStateID = {0}", ustID);
                        n++;
                    }
                    else
                    {
                        strSQL += string.Format(" or ApprovalStateID = {0}", ustID);
                        n++;
                    }
        ...
        protected void chk_Allselect_CheckedChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grdAskLeaveApprovalState.Rows.Count; i++)
            {
                ...
                ((CheckBox)grdAskLeaveApprovalState.Rows[i].FindControl("chk_Select")).Checked = ck.Checked;
            }
        }

        protected void grdAskLeaveApprovalState_SelectedIndexChanged(object sender, EventArgs e)
        {
